...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            newStatesValues = self.values.copy()

            for state in self.mdp.getStates():
                qValuesList = []

                # if self.mdp.isTerminal(state): # not really needed since counter give 0 by default
                #     newStatesValues[state] = 0.
                if not self.mdp.isTerminal(state):
                    for action in self.mdp.getPossibleActions(state):
                        qValuesList.append(self.computeQValueFromValues(state, action))

                    newStatesValues[state] = max(qValuesList)
            self.values = newStatesValues
    ...
    def computeActionFromValues(self, state):
        ...
        # actionsAndQValues = util.Counter()    #more elegant, but might cause trouble
        #
        # for action in self.mdp.getPossibleActions(state):
        #     actionsAndQValues[action] = self.getQValue(state, action)
        #
        # return actionsAndQValues.argMax()
