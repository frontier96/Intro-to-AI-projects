...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return (self.startingPosition, tuple(corners_pos))
...
class FoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        # self.heuristicInfo = {'state': startingGameState}
...
def foodHeuristic(state, problem):
    ...
    if (len(foodList) >= 1):
        farthest_food_d = max([mazeDistance(position, food, current_state) for food in foodList])
    #     return mazeDistance(position, foodList[0], current_state)
    # if (len(foodList) >= 2):
    #     closest_food_d, closest_food = min([(util.manhattanDistance(position, food), food)
    #                                         for food in foodList])
    #     foodList.remove(closest_food)
    #     farthest_food_d, farthest_food = max([(util.manhattanDistance(position, food), food)
    #                                           for food in foodList])
    #
    #     #currentNode = problem.startingGameState
    #     # dis_to_closest = mazeDistance(position, closest_food, currentNode)
    #     closest_to_farthest = mazeDistance(closest_food, farthest_food, current_state)
    #
    #     # total_h = dis_to_closest + closest_to_farthest
        total_h = farthest_food_d
    return total_h
